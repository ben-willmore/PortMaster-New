...
# Turn off dynarec for one memory address, which prevents the game
# from starting on rocknix panfrost/adreno
if [[ "$CFW_NAME" = "ROCKNIX" ]] && \
    [[ ! -z `glxinfo | grep "OpenGL version string"` ]]; then
  case `md5sum "$GAMEDATA/$BINARY" | awk '{print $1}'` in
  59c6bdc7817370d42d71219c099e9049)
    export BOX86_NODYNAREC=0x083aa402-0x083aa403
    ;;
  36afb5392d2ae8b42f14949a835e977a)
